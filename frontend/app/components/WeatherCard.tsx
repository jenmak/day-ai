...
export const WeatherCard = ({ weather }: { weather: Weather }) => {
    ...
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <span className="truncate flex-1 text-left cursor-help">
  ...
  return (
    <div className="weather">
      ...
      <h2
        className={`text-center text-lg md:text-2xl font-bold mb-4 text-white flex flex-row justify-between weather`}
      >
        {weatherConditionWithTooltip()}
        <div className="flex flex-row justify-center flex-shrink-0 ml-4">
          <p className="font-body font-normal text-white/80">
            {weather.temperatureRange.temperatureMinimum}
          </p>
          <p className="font-body font-thin">/</p>
          <p className="font-body font-semibold text-white">
            {weather.temperatureRange.temperatureMaximum}°F
          </p>
        </div>
      </h2>
      {/* Weather card */}
        ...
        <CardHeader className="relative text-black z-20">
          <div className="absolute top-0 left-3 md:top-3 md:left-5 flex flex-col gap-4">
            <div className="flex flex-col items-start">
              <CardTitle className="font-light font-sm md:font-body mb-1">
                {getDate(weather.date)}
              </CardTitle>
              <CardDescription
                className={`font-heading font-medium text-black mb-2 text-xl md:text-2xl`}
              >
                {dayOfWeek}
              </CardDescription>
            </div>
            <div className="flex flex-row items-center gap-2">
              ...
              <p className="font-body font-light text-black/80 text-sm md:text-base">
            ...
            <div className="flex flex-row items-center gap-2">
              <span className="relative bg-white/60 backdrop-blur-sm rounded-full p-2">
                <Image
                  ...
                  alt="Percentage of rain"
                  className="w-5 h-5 md:w-9 md:h-9"
                />
              </span>
              <p className="font-body font-light text-black/80 text-sm md:text-base">
                {weather.rainProbabilityPercentage}% rain
              </p>
            </div>
          </div>
        </CardHeader>
