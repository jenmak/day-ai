...
export function getPlaceErrorState(
  ...
  if (error) {
    return {
      errorMessage: error instanceof Error ? error.message : "An unknown error occurred. Please try again.",
      hasError: true
    }
  }
  ...
  if (!isLoading && place && !place.weather) {
    return {
      errorMessage: "The place you searched for does not contain any weather data. Please try a different location.",
      hasError: true
    }
  }
