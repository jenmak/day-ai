...
var ClothingService = class {
  ...
  static getWeatherAppropriatenessScore(
    ...
    if (temperature >= 85) {
      if (
        ["t-shirt", "tank-top", "shorts", "sandals", "dress", "skirt"].includes(clothingCategory)
      ) {
        score += 80
      ...
      if (
        ["coat", "winter-coat", "boots", "winter-boots", "gloves", "scarf"].includes(
          clothingCategory
        )
      ) {
        score += 95
...
var DESCRIPTION_TO_NORMALIZED_PLACE_PROMPT = /* @__PURE__ */ __name(
  (
    description
...
Respond only with valid JSON:`,
  "DESCRIPTION_TO_NORMALIZED_PLACE_PROMPT"
)
...
var places = router({
  ...
  getByNormalizedPlace: procedure
    .input(z3.object({ normalizedPlace: z3.string() }))
