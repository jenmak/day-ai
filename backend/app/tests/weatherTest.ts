...
export async function testWeatherService() {
  ...
  try {
    ...
    forecast.forEach((weather, index) => {
      ...
      console.log(`  Range: ${weather.temperatureRange.temperatureMinimum}°F - ${weather.temperatureRange.temperatureMaximum}°F`)
    ...
    console.log(`✅ Today's weather: ${todayWeather.degreesFahrenheit}°F, ${todayWeather.condition}`)
...
export async function testWeatherServiceMultipleLocations() {
  ...
  for (const location of testLocations) {
    try {
      ...
      console.error(`❌ ${location.name}: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
}
